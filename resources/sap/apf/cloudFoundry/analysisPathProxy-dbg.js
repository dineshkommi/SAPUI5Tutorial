...
sap.ui.define([], function() {
	...
	var AnalysisPathProxy = function(inject) {
		...
		this.openPath = function(analysisPathId, callback) {
			var url = analysisPathsUri + "/" + analysisPathId + "?$select=AnalysisPathName,SerializedAnalysisPath";
			ajax({
				method : "GET",
				url : url,
				dataType : "json",
				success : function(result /*, textStatus, xhr*/) {
					var data = {
							AnalysisPathName : result.analysisPathName,
							SerializedAnalysisPath : result.serializedAnalysisPath //TODO check in runtime, whether JSON.parse needs to be applied
					};
		...
		this.createPath = function(analysisPathName, callback, serializableApfState) {
			...
			var data = {
					...
					structuredAnalysisPath : structuredAnalysisPath,
					serializedAnalysisPath : serializableApfState
			};
			ajax({
				method : "POST",
				url : analysisPathsUri,
		...
		this.modifyPath = function(analysisPathId, analysisPathName, callback, serializableApfState) {
			...
			ajax({
				method : "PUT",
				url : analysisPathsUri + "/" + analysisPathId,
				data : JSON.stringify(data),
				dataType : "json",
				success : function(/* result, textStatus, xhr*/) {
		...
		this.readPaths = function(callback) {
				...
				if (result.analysisPaths !== null) {
					result.analysisPaths.forEach(function(path){
						paths.push({
							...
							StructuredAnalysisPath : path.structuredAnalysisPath
						});
					});
				}
				return paths;
			...
			var url = analysisPathsUri + "?$select=AnalysisPath,AnalysisPathName,LastChangeUtcDateTime,StructuredAnalysisPath&$filter=(AnalyticalConfiguration%20eq%20'"
