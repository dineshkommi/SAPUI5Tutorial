	...
	var TextRule = RuleBase.extend("sap.rules.ui.TextRule", {
		...
		_addConditionBlock: function(oEvent, sType) {
			...
			mParameters.success = function(newCondition){
				that._handleVerticalLayoutDataRequested()
                var data = {};
            ...
            oDataModel.read(sPath,{
				...
				success: function (response) {
                    ...
                    if (response.Type === that.typeElse){
                        ...
                        var oElsePanel = that._createElseFormLayout( oContext, that.oBundle.getText("titleElse"));
                        ...
                        var oElseIfPanel = that._createFormLayout( oContext, that.oBundle.getText("titleElseIf"));
                        if (data.bfirst) {
                            oVerticalLayout.removeContent(data.nIndex);
                    ...
                    that._dataPartReceived(dataPartName.verticalLayout);
                },
                error: function () {
		...
		_bindRule: function() {
			...
			this._handleRuleDataRequested();
			this._handleVerticalLayoutDataRequested();
			if (sBindingPath && oModel) {
				...
				oModel.submitChanges({
					...
					success: function(data) {
						...
						that._handleRuleDataReceived(data);
		 ...
		 */
		_createFormLayout: function(oContext, sTitle) {
			var that = this;
			var oPanel = new Panel({
				...
				content: new Form({
					...
					formContainers: [
						// If form container
						this._createIfBlockFormContainer(oContext, sTitle),
						// Then form container
		 ...
		 */
		_createElseFormLayout: function(oContext, sTitle) {
			var that = this;
			var oToolbar = new Toolbar({
				design: "Transparent"
			});
		...
		_getElseIfPanel: function() {
			...
			if (oElseIfData.length > 0) {
				for (var i = 0; i < oElseIfData.length; i++) {
					...
					oControls.push(this._createFormLayout( oContext, sTitle));
				}
			} else {
				var oPanel = new Panel({
					headerText: sTitle,
		...
		_getElsePanel: function() {
			...
			if (oElseData.length > 0) {
				...
				oControls.push(this._createElseFormLayout( oContext, sTitle));
			} else {
				var oPanel = new Panel( {
					headerText: sTitle,
					visible: {
		...
		_getIfPanel: function() {
			...
			return this._createFormLayout( oContext, sTitle);
		},
        ...
        /*
         * Updates Rule Data status as pending
         */
        ...
        /*
         * Updates Rule Data status as received
         */
        ...
        /*
         * Updates TextRuleCondition Data status as pending
         */
		_handleVerticalLayoutDataRequested: function() {
			this._dataPartRequested(dataPartName.verticalLayout);
		},

        /*
         * Updates Vocabulary Data status as changed
         */
        ...
        /*
         * Updates Vocabulary Data status as pending
         */
        ...
        /*
         * Updates Vocabulary Data status as received
         */
        ...
        /*
         * Returns true if Rule, Vocabulary and Condition data has been received
         */
